  ...
  WITH РекурсивныйЗапрос AS
   (
      SELECT 0 AS Уровень, Ссылка
           , КодРодителя = Код
           , КодТовара   = Код
           , Наименование, ЭтоЭлемент
        FROM Справочник.Номенклатура AS T
       WHERE Код = @КодГруппы

      UNION ALL

      SELECT Предыдущий.Уровень + 1 AS Уровень, Текущий.Ссылка
   ...
   SELECT Уровень, Ссылка
        ...
        , КодТовара
        , Наименование
        , ЭтоГруппа = CASE WHEN ЭтоЭлемент = FALSE THEN 'да' ELSE 'нет' END
     INTO @Таблица
     FROM РекурсивныйЗапрос
    ORDER BY Уровень, ЭтоЭлемент DESC
